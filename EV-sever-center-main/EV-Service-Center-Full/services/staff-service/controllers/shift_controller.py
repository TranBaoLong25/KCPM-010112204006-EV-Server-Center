...
def bulk_schedule_shifts():
    try:
        ...
        if start_date > end_date:
            return jsonify({"success": False, "error": "start_date không được lớn hơn end_date"}), 400

        shifts_data = data.get("shifts")
        if not isinstance(shifts_data, list) or len(shifts_data) == 0:
            return jsonify({"success": False, "error": "shifts phải là danh sách không rỗng"}), 400

        created_shifts = []

        for shift_data in shifts_data:
            required_shift_fields = ["shift_date", "shift_type", "start_time", "end_time"]
            if not all(field in shift_data for field in required_shift_fields):
                return jsonify({"success": False, "error": "Shift item thiếu required fields"}), 400

            shift_date = datetime.fromisoformat(shift_data["shift_date"]).date()

            if shift_date < start_date or shift_date > end_date:
                return jsonify({"success": False, "error": "shift_date nằm ngoài khoảng start_date/end_date"}), 400

            shift = StaffShift(
                staff_id=staff_id,
                shift_date=shift_date,
                shift_type=shift_data["shift_type"],
                start_time=datetime.strptime(shift_data["start_time"], "%H:%M:%S").time(),
                end_time=datetime.strptime(shift_data["end_time"], "%H:%M:%S").time(),
        ...
        return jsonify({
            ...
            "shifts": [shift.to_dict() for shift in created_shifts]
        }), 201
